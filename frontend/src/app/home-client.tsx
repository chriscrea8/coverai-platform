...
export default function HomePage() {
  ...
  const features = [
    ...
    { icon: '📋', title: 'Smart Policy Matching', desc: 'Answer 4 questions and our AI recommends the exact coverage your business needs.', color: '#F4A623' },
    ...
    <div className="noise-overlay min-h-screen">
      ...
      <section className="gradient-hero min-h-screen flex flex-col items-center justify-center text-center px-6 pt-20 pb-16">
        ...
        <div className="flex gap-4 flex-wrap justify-center">
          <Link href="/chat" className="px-8 py-4 rounded-xl bg-accent text-ink font-syne font-bold text-base hover:bg-yellow-400 transition-all hover:-translate-y-0.5"
            style={{ boxShadow: '0 8px 32px rgba(244,166,35,0.3)' }}>
            Talk to AI Assistant
          </Link>
          <Link href="/coverage" className="px-8 py-4 rounded-xl text-white font-syne font-semibold text-base hover:bg-white/10 transition-all"
        ...
        <div className="mt-10 md:mt-16 grid grid-cols-3 max-w-xs md:max-w-xl w-full rounded-2xl overflow-hidden"
          ...
          {[['10,000+', 'Businesses Protected'], ['₦2.4B', 'Claims Processed'], ['98%', 'Satisfaction Rate']].map(([n, l]) => (
            <div key={l} className="py-5 md:py-6 px-3 md:px-4 text-center" style={{ background: 'rgba(13,27,62,0.6)' }}>
              <div className="font-syne font-black text-xl md:text-3xl">{n}</div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5 max-w-6xl mx-auto">
          {features.map(f => (
            <div key={f.title} className="p-8 rounded-2xl transition-all hover:-translate-y-1"
              ...
              <div className="w-12 h-12 rounded-xl flex items-center justify-center text-2xl mb-5" style={{ background: f.color + '20' }}>{f.icon}</div>
          ...
          {steps.map((s, i) => (
            <div key={s.title} className="text-center">
              <div className="w-16 h-16 rounded-full flex items-center justify-center text-2xl mx-auto mb-4 relative"
                ...
                {s.icon}
                <span className="absolute -top-1 -right-1 w-5 h-5 rounded-full bg-accent text-ink text-xs font-black flex items-center justify-center">{i + 1}</span>
              </div>
      ...
      <section id="pricing" className="py-24 px-6" style={{ background: '#0D1B3E' }}>
        <div className="text-center mb-14">
          ...
          <p className="text-muted mt-3">No hidden fees. Cancel anytime. <Link href="/pricing" className="text-accent hover:underline">See full details →</Link></p>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 max-w-5xl mx-auto">
          {[
            ...
            <div key={p.name} className={`p-9 rounded-2xl relative ${(p as any).featured ? 'md:scale-105' : ''}`}
              ...
              <div className="text-muted text-sm mb-6">{p.desc}</div>
              <div className="font-syne font-black text-4xl mb-1">{p.price}</div>
              <div className="text-muted text-sm mb-7">{p.per}</div>
              <ul className="space-y-3 mb-8">{p.features.map(f => <li key={f} className="text-sm flex gap-2"><span className="text-teal font-bold">✓</span>{f}</li>)}</ul>
