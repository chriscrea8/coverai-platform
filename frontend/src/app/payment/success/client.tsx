...
export default function PaymentSuccessClient() {
  ...
  if (stage === 'verifying') return (
    <div className="min-h-screen flex flex-col items-center justify-center px-4" style={{ background: '#080D1A' }}>
    ...
    <div className="min-h-screen flex flex-col items-center justify-center px-4" style={{ background: '#080D1A' }}>
      <div className="text-center max-w-sm">
        ...
        <div className="flex gap-2 justify-center mb-6">
          {[0,1,2].map(i => (
            <div key={i} className="w-2 h-2 rounded-full animate-bounce" style={{ background: '#F4A623', animationDelay: `${i * 0.2}s` }} />
          ))}
        </div>
      ...
      <div className="text-center max-w-sm">
        ...
        <div className="flex gap-3 justify-center">
          <Link href="/dashboard" className="px-5 py-2.5 rounded-xl text-sm font-bold"
            style={{ background: 'rgba(255,255,255,.08)', color: '#fff' }}>Dashboard</Link>
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center px-4 py-10" style={{ background: '#080D1A' }}>
        ...
        {[...Array(12)].map((_, i) => (
          <div key={i} className="absolute w-2 h-2 rounded-full animate-bounce opacity-20"
            style={{
              ...
              background: ['#2EC97E','#F4A623','#7C6BFF','#00C2A8'][i % 4],
      ...
      <div className="relative w-full max-w-md">
        ...
        <div className="text-center mb-8">
          ...
          <div className="relative inline-flex items-center justify-center w-20 h-20 mx-auto mb-5">
            <div className="absolute inset-0 rounded-full animate-ping opacity-20" style={{ background: '#2EC97E' }} />
          ...
          <h1 className="font-syne font-black text-2xl md:text-3xl mb-2" style={{ color: '#2EC97E' }}>
            You're Covered! 🎉
          </h1>
          <p className="text-muted text-sm">
            Payment confirmed. Your policy is{' '}
            {policy?.policyStatus === 'active' ? (
              <span style={{ color: '#2EC97E' }}>active</span>
            ) : (
              <span style={{ color: '#F4A623' }}>being activated</span>
            )}.
          </p>
        </div>

        {/* Policy card */}
        <div className="rounded-2xl overflow-hidden mb-5" style={{ background: 'rgba(13,27,62,.9)', border: '1px solid rgba(255,255,255,.1)' }}>
          {/* Card header */}
          <div className="px-5 py-4 flex items-center justify-between"
            ...
            <span className="px-2.5 py-1 rounded-full text-xs font-bold"
              style={{ background: policy?.policyStatus === 'active' ? 'rgba(46,201,126,.2)' : 'rgba(244,166,35,.2)', color: policy?.policyStatus === 'active' ? '#2EC97E' : '#F4A623' }}>
            ...
            {[
              ...
              <div key={item.label} className="p-3 rounded-xl" style={{ background: 'rgba(255,255,255,.04)' }}>
                ...
                <div className={`text-sm font-semibold ${item.mono ? 'font-mono text-xs' : ''}`}
                  style={item.highlight ? { color: '#2EC97E' } : {}}>
                  {item.value}
                </div>
              </div>
            ))}
          ...
          <div className="px-5 pb-5">
            <div className="p-3 rounded-xl" style={{ background: 'rgba(244,166,35,.08)', border: '1px solid rgba(244,166,35,.2)' }}>
              <div className="flex items-center gap-2 mb-1">
                <span>📅</span>
                <span className="text-xs font-semibold" style={{ color: '#F4A623' }}>Renewal Reminder</span>
              </div>
        ...
        {policy?.policyDetails?.answers && (
          <div className="rounded-2xl p-5 mb-5" style={{ background: 'rgba(13,27,62,.8)', border: '1px solid rgba(255,255,255,.07)' }}>
        ...
        {/* Next steps */}
        <div className="rounded-2xl p-5 mb-6" style={{ background: 'rgba(13,27,62,.8)', border: '1px solid rgba(255,255,255,.07)' }}>
