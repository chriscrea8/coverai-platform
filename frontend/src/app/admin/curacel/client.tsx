    ...
    <div style={{ minHeight: '100vh', background: 'var(--ink)', color: '#fff' }}>
      ...
      <div style={{ padding: '16px 24px', borderBottom: '1px solid rgba(255,255,255,0.07)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: 'rgba(10,15,30,0.95)', position: 'sticky', top: 0, zIndex: 10 }}>
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          ...
          <button onClick={async () => { try { const r = await (await import('@/lib/api')).curacelApi.syncPolicies(); alert(r.data?.message || 'Synced!'); } catch {} }} style={{ padding: '8px 16px', borderRadius: 10, background: 'rgba(0,194,168,0.15)', border: '1px solid rgba(0,194,168,0.3)', color: 'var(--teal)', cursor: 'pointer', fontSize: 13 }}>⟳ Sync Policies</button>
      ...
      <div style={{ maxWidth: 1100, margin: '0 auto', padding: '28px 24px' }}>
        ...
        <div style={{ padding: 24, borderRadius: 20, marginBottom: 28, background: isConnected ? 'rgba(46,201,126,0.1)' : 'rgba(244,166,35,0.1)', border: `1px solid ${isConnected ? 'rgba(46,201,126,0.3)' : 'rgba(244,166,35,0.3)'}` }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
            ...
            <div style={{ flex: 1 }}>
              ...
              <div style={{ color: 'var(--muted)', fontSize: 14 }}>{status?.message || 'Loading...'}</div>
            </div>
            {!isConnected && (
              <div style={{ padding: '12px 20px', borderRadius: 12, background: 'rgba(244,166,35,0.15)', border: '1px solid rgba(244,166,35,0.3)', fontSize: 13 }}>
                <div style={{ fontWeight: 700, color: 'var(--accent)', marginBottom: 4 }}>To activate live products:</div>
                <div style={{ color: 'var(--muted)' }}>1. Get API key from <strong>grow@curacel.ai</strong></div>
                <div style={{ color: 'var(--muted)' }}>2. Add <code style={{ background: 'rgba(255,255,255,0.1)', padding: '1px 6px', borderRadius: 4 }}>CURACEL_API_KEY</code> to Railway</div>
                <div style={{ color: 'var(--muted)' }}>3. Optionally set <code style={{ background: 'rgba(255,255,255,0.1)', padding: '1px 6px', borderRadius: 4 }}>CURACEL_ENV=production</code></div>
              </div>
            )}
            {isConnected && wallet && (
              <div style={{ padding: '12px 20px', borderRadius: 12, background: 'rgba(46,201,126,0.1)', border: '1px solid rgba(46,201,126,0.3)', textAlign: 'center' }}>
                <div style={{ fontSize: 11, color: 'var(--muted)', marginBottom: 4 }}>Wallet Balance</div>
                <div style={{ fontFamily: 'Syne, sans-serif', fontWeight: 900, fontSize: 22, color: 'var(--green)' }}>₦{Number(wallet.balance || 0).toLocaleString()}</div>
              </div>
            )}
          </div>
        </div>
        ...
        <div style={{ padding: 20, borderRadius: 16, marginBottom: 28, background: 'rgba(124,107,255,0.1)', border: '1px solid rgba(124,107,255,0.3)' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
            <div style={{ padding: '12px 16px', borderRadius: 10, background: 'rgba(255,255,255,0.04)' }}>
              <div style={{ fontSize: 11, color: 'var(--muted)', marginBottom: 4 }}>Set this URL in your Curacel dashboard</div>
            ...
            <div style={{ padding: '12px 16px', borderRadius: 10, background: 'rgba(255,255,255,0.04)' }}>
              <div style={{ fontSize: 11, color: 'var(--muted)', marginBottom: 4 }}>Add to Railway env vars</div>
              <div style={{ fontSize: 12, color: 'var(--muted)' }}><code style={{ color: 'var(--accent)' }}>CURACEL_WEBHOOK_SECRET</code> — from Curacel dashboard</div>
              <div style={{ fontSize: 12, color: 'var(--muted)', marginTop: 4 }}><code style={{ color: 'var(--accent)' }}>CURACEL_PARTNER_KEY</code> — your partner key UUID</div>
            </div>
          </div>
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 16, marginBottom: 28 }}>
          {[
            ...
            { label: 'Insurance Types', value: productTypes.length, icon: '📊', color: 'var(--accent)' },
            { label: 'Insurers', value: Array.from(new Set(products.map((p: any) => p.insurer?.name).filter(Boolean))).length, icon: '🏛️', color: 'var(--green)' },
          ].map(s => (
        ...
        <div style={{ marginBottom: 20 }}>
          ...
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
            <button onClick={() => loadAll()} style={{ padding: '7px 14px', borderRadius: 20, fontSize: 12, fontWeight: 600, cursor: 'pointer', border: 'none', fontFamily: 'inherit', background: !selectedType ? 'var(--accent)' : 'rgba(255,255,255,0.06)', color: !selectedType ? 'var(--ink)' : 'var(--muted)' }}>All</button>
            {productTypes.map((t: any) => (
              <button key={t.id} onClick={() => loadByType(String(t.id))} style={{
                ...
                background: selectedType === String(t.id) ? 'var(--accent)' : 'rgba(255,255,255,0.06)',
                color: selectedType === String(t.id) ? 'var(--ink)' : 'var(--muted)',
              }}>{t.name}</button>
            ))}
          </div>
        </div>
        ...
        {loading ? (
          <div style={{ textAlign: 'center', padding: 60, color: 'var(--muted)' }}>Loading products from Curacel...</div>
        ) : (
              ...
              <div key={p.id || p.code} style={{ padding: '18px 20px', borderRadius: 16, background: 'rgba(13,27,62,0.7)', border: '1px solid rgba(255,255,255,0.08)' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 10 }}>
                  ...
                  <div style={{ textAlign: 'right', flexShrink: 0, marginLeft: 12 }}>
                    <div style={{ fontFamily: 'Syne, sans-serif', fontWeight: 900, color: 'var(--accent)', fontSize: 15 }}>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 11, color: '#4A5568', borderTop: '1px solid rgba(255,255,255,0.05)', paddingTop: 8 }}>
                  <span>Code: <code style={{ color: 'var(--muted)' }}>{p.code}</code></span>
                  <span style={{ color: 'var(--green)' }}>Commission: {p.partner_commission_rate}%</span>
                </div>
              </div>
            ))}
          </div>
          ...
          <div style={{ marginTop: 40, padding: 28, borderRadius: 20, background: 'rgba(13,27,62,0.6)', border: '1px solid rgba(255,255,255,0.08)' }}>
            ...
            {[
              ...
              <div key={s.step} style={{ display: 'flex', gap: 16, marginBottom: 14, padding: '12px 16px', borderRadius: 10, background: 'rgba(255,255,255,0.03)' }}>
                <div style={{ width: 28, height: 28, borderRadius: '50%', background: 'var(--accent)', color: '#fff', fontWeight: 900, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, fontSize: 13 }}>{s.step}</div>
                <div>
