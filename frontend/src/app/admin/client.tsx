...
const STATUS_STYLES: Record<string, [string, string]> = {
  active:       ['rgba(46,201,126,.15)',  'var(--green)'],
  approved:     ['rgba(46,201,126,.15)',  'var(--green)'],
  paid:         ['rgba(46,201,126,.15)',  'var(--green)'],
  successful:   ['rgba(46,201,126,.15)',  'var(--green)'],
  pending:      ['rgba(244,166,35,.15)',  'var(--accent)'],
  submitted:    ['rgba(244,166,35,.15)',  'var(--accent)'],
  under_review: ['rgba(124,107,255,.15)', '#7C6BFF'],
  processing:   ['rgba(124,107,255,.15)', '#7C6BFF'],
  rejected:     ['rgba(232,69,69,.15)',   'var(--red)'],
  expired:      ['rgba(232,69,69,.15)',   'var(--red)'],
  cancelled:    ['rgba(232,69,69,.15)',   'var(--red)'],
  inactive:     ['rgba(232,69,69,.15)',   'var(--red)'],
  suspended:    ['rgba(232,69,69,.15)',   'var(--red)'],
  lapsed:       ['rgba(232,69,69,.15)',   'var(--red)'],
}

function Badge({ status }: { status: string }) {
  const [bg, color] = STATUS_STYLES[status?.toLowerCase()] || ['rgba(132,146,180,.15)', 'var(--muted-light)']
  return (
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 md:p-6"
      ...
      <div className={`w-full ${wide ? 'max-w-2xl' : 'max-w-md'} rounded-2xl p-5 md:p-6 max-h-[90vh] overflow-y-auto`}
        style={{ background: 'var(--navy)', border: '1px solid rgba(255,255,255,.15)' }}
...
function ActionBtn({ label, color, onClick, disabled = false }: any) {
  const schemes: Record<string, [string, string, string]> = {
    green:  ['rgba(46,201,126,.15)',  'var(--green)', 'rgba(46,201,126,.3)'],
    red:    ['rgba(232,69,69,.15)',   'var(--red)', 'rgba(232,69,69,.3)'],
    purple: ['rgba(124,107,255,.15)', '#7C6BFF', 'rgba(124,107,255,.3)'],
    ghost:  ['rgba(255,255,255,.07)', 'var(--muted-light)', 'transparent'],
    yellow: ['rgba(244,166,35,.15)',  'var(--accent)', 'rgba(244,166,35,.3)'],
  }
      ...
      <header className="flex items-center justify-between px-4 md:px-6 py-3 sticky top-0 z-40"
        ...
        <div className="flex items-center gap-3">
          ...
          <span className="px-2 py-0.5 rounded text-xs font-black tracking-wider"
            style={{ background: 'rgba(232,69,69,.2)', color: 'var(--red)', border: '1px solid rgba(232,69,69,.35)' }}>ADMIN</span>
        </div>
        <div className="flex items-center gap-3">
      ...
      <div className="fixed top-16 left-1/2 -translate-x-1/2 z-50 flex flex-col gap-2 items-center pointer-events-none">
        {toastQ.map(t => (
          <div key={t.id} className="px-4 py-2.5 rounded-xl text-sm font-semibold shadow-xl whitespace-nowrap"
            style={{ background: t.ok ? 'rgba(46,201,126,.2)' : 'rgba(232,69,69,.2)', border: `1px solid ${t.ok ? 'rgba(46,201,126,.4)' : 'rgba(232,69,69,.4)'}`, color: t.ok ? 'var(--green)' : 'var(--red)', backdropFilter: 'blur(8px)' }}>
            {t.ok ? '✓' : '✕'} {t.msg}
          </div>
        ))}
      </div>
          ...
          {TABS.map(t => (
            <button key={t.id} onClick={() => setTab(t.id)}
              ...
              {t.id === 'claims' && openClaims > 0 && (
                <span className="ml-auto text-xs font-black px-1.5 py-0 rounded-full" style={{ background: 'var(--red)', color: '#fff' }}>{openClaims}</span>
              )}
            </button>
          ))}
        </aside>
          ...
          <button key={t.id} onClick={() => setTab(t.id)}
            ...
            {t.id === 'claims' && openClaims > 0 && (
              <span className="absolute top-1 right-1 w-3.5 h-3.5 rounded-full text-[8px] font-black flex items-center justify-center"
                style={{ background: 'var(--red)', color: '#fff' }}>{openClaims}</span>
            )}
          </button>
        ))}
      </nav>
...
function OverviewTab({ store, loading, setTab }: any) {
  ...
  const kpis = [
    { label: 'Total Users',     value: s.totalUsers,     sub: 'registered accounts', icon: '👥', color: 'var(--teal)', tab: 'users'     },
    { label: 'Active Policies', value: s.activePolicies, sub: `of ${s.totalPolicies ?? '?'} total`, icon: '📋', color: 'var(--accent)', tab: 'policies'  },
    { label: 'Open Claims',     value: s.openClaims,     sub: 'awaiting review',     icon: '⚠️', color: 'var(--red)', tab: 'claims'    },
    { label: 'Total Revenue',   value: s.totalRevenue != null ? `₦${Number(s.totalRevenue).toLocaleString()}` : null, sub: 'from payments', icon: '💰', color: 'var(--green)', tab: 'analytics' },
    { label: 'Providers',       value: s.totalProviders, sub: 'insurance partners', icon: '🏦', color: '#7C6BFF', tab: 'providers' },
    { label: 'Products',        value: s.totalProducts,  sub: 'coverage types',     icon: '📦', color: 'var(--teal)', tab: 'products'  },
  ]
    ...
    <div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 gap-3 mb-6">
        {kpis.map(k => (
          <Card key={k.label} onClick={() => setTab(k.tab)} className="hover:-translate-y-0.5">
            ...
            {busy
              ? <div className="h-7 w-20 rounded-lg animate-pulse mb-1" style={{ background: 'var(--border)' }} />
      ...
      <Card className="mb-4">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          {[
            { label: 'Review Claims',  icon: '🛡️', tab: 'claims',    note: `${s.openClaims || 0} open`, accent: 'var(--red)' },
            { label: 'Leads Dashboard', icon: '📊', href: '/admin/leads', note: 'Track conversions', accent: 'var(--teal)' },
            ...
            { label: 'Add Provider',   icon: '🏦', tab: 'providers', note: 'Onboard insurer', accent: 'var(--accent)' },
            { label: 'Add Product',    icon: '📦', tab: 'products',  note: 'New coverage',   accent: 'var(--teal)' },
      ...
      <div className="grid md:grid-cols-2 gap-4">
        <Card>
          ...
          {[
            ['Active',  s.activePolicies, 'var(--green)'],
            ['Pending', (s.totalPolicies ?? 0) - (s.activePolicies ?? 0), 'var(--accent)'],
            ['Total',   s.totalPolicies,  'var(--teal)'],
          ].map(([l, v, c]: any) => (
        ...
        <Card>
          ...
          {[
            ['Submitted',   s.openClaims,  'var(--accent)'],
      ...
      <div className="flex gap-2 flex-wrap mb-5 text-xs">
        {counts.map(([s, n]) => (
          <span key={s as string} className="px-2 py-1 rounded-lg capitalize"
            style={{ background: 'rgba(255,255,255,.05)', color: 'var(--muted-light)' }}>
        ...
        <Modal title={`Claim — ${selected.claimNumber}`} wide onClose={() => { setSelected(null); setNote('') }}>
            ...
            <>
              ...
              <div className="flex gap-3 mt-4">
                ...
                <button onClick={() => act(selected.id, 'reject')} disabled={!!busy}
                  className="flex-1 py-3 rounded-xl text-sm font-bold flex items-center justify-center gap-2 disabled:opacity-50 transition-all"
                  style={{ background: 'rgba(232,69,69,.2)', color: 'var(--red)', border: '1px solid rgba(232,69,69,.4)' }}>
          ...
          {selected.status === 'approved' && (
            <div className="mt-4 p-4 rounded-xl" style={{ background: 'rgba(46,201,126,.07)', border: '1px solid rgba(46,201,126,.2)' }}>
              <p className="text-sm font-semibold mb-1" style={{ color: 'var(--green)' }}>✅ Claim Approved — Payout Pending</p>
...
function ProvidersTab({ store, token, loading, toast, reload }: any) {
  ...
  function SyncBadge({ p }: { p: any }) {
    ...
    const configs: Record<string, { color: string, bg: string, label: string }> = {
      success: { color: 'var(--teal)', bg: 'rgba(0,194,168,.1)', label: `✓ ${p.syncedProductCount || 0} products` },
      error:   { color: '#F56565', bg: 'rgba(245,101,101,.1)', label: '✗ Sync failed' },
      syncing: { color: 'var(--accent)', bg: 'rgba(244,166,35,.1)', label: '⟳ Syncing…' },
    ...
    <div>
      ...
      <div className="flex gap-2 flex-wrap mb-5 text-xs">
        {Object.entries(counts).map(([k, v]) => (
          <span key={k} className="px-2 py-1 rounded-lg capitalize" style={{ background: 'rgba(255,255,255,.05)', color: 'var(--muted-light)' }}>
          ...
          <div className="flex gap-3">
            ...
            <button onClick={() => { toggleSuspend(viewing); setViewing(null) }}
              ...
              style={viewing.status === 'suspended'
                ? { background: 'rgba(46,201,126,.15)', color: 'var(--green)', border: '1px solid rgba(46,201,126,.3)' }
                : { background: 'rgba(232,69,69,.15)', color: 'var(--red)', border: '1px solid rgba(232,69,69,.3)' }}>
            ...
            {[
              ...
              <button key={r.role} onClick={() => changeRole(roleTarget.id, r.role)}
                ...
                style={{
                  ...
                  border: `1px solid ${roleTarget.role === r.role ? 'rgba(244,166,35,.4)' : 'var(--border)'}`,
                }}>
                <div className="flex items-center gap-3">
...
function PoliciesTab({ store, token, loading, toast, reload }: any) {
  ...
  return (
    <div>
      ...
      <div className="flex gap-2 flex-wrap mb-5 text-xs">
        {counts.map(([s, n]) => n ? (
          <span key={s as string} className="px-2 py-1 rounded-lg capitalize" style={{ background: 'rgba(255,255,255,.05)', color: 'var(--muted-light)' }}>
                ...
                <div className="flex flex-wrap justify-between gap-3">
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-2 mb-1.5 flex-wrap">
                      ...
                      {p.autoRenew && <span className="text-xs px-2 py-0.5 rounded-full" style={{ background: 'rgba(0,194,168,.1)', color: 'var(--teal)' }}>Auto-renew</span>}
                    </div>
                    <div className="flex gap-4 flex-wrap text-xs text-muted">
          ...
          <div className="flex gap-3">
            {viewing.policyStatus === 'pending' && (
              <button onClick={() => { activate(viewing.id); setViewing(null) }}
                className="flex-1 py-2.5 rounded-xl text-sm font-bold"
                style={{ background: 'var(--green)', color: '#fff' }}>✓ Activate Policy</button>
            )}
...
function AnalyticsTab({ store, token, loading, toast, reload }: any) {
  ...
  const revenueKpis = [
    { label: 'Total Revenue',     value: s.totalRevenue    != null ? `₦${Number(s.totalRevenue).toLocaleString()}`    : '—', icon: '💰', color: 'var(--green)', hint: 'Successful payments' },
    { label: 'Gross Premiums',    value: summary.totalGross != null ? `₦${Number(summary.totalGross).toLocaleString()}` : '—', icon: '📊', color: 'var(--accent)', hint: 'Total premium volume' },
    { label: 'Commissions Earned',value: summary.totalCommission != null ? `₦${Number(summary.totalCommission).toLocaleString()}` : '—', icon: '💵', color: 'var(--teal)', hint: 'Gross commissions' },
  ...
  const ledgerStatusCards = [
    { key: 'pending',    label: 'Pending Payout',  color: 'var(--accent)', value: summary.totalPending    != null ? `₦${Number(summary.totalPending).toLocaleString()}`    : '—', count: summary.pendingCount ?? 0 },
    { key: 'processing', label: 'Processing',       color: '#7C6BFF', value: summary.totalNet != null ? '—' : '—', count: summary.processingCount ?? 0 },
    { key: 'paid',       label: 'Paid Out',         color: 'var(--green)', value: summary.totalPaid       != null ? `₦${Number(summary.totalPaid).toLocaleString()}`       : '—', count: summary.paidCount ?? 0 },
  ]
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-4">
        {revenueKpis.map(k => (
          <Card key={k.label}>
            ...
            {busy ? <div className="h-7 w-20 rounded-lg animate-pulse mb-0.5" style={{ background: 'var(--border)' }} />
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 gap-3 mb-6">
        {[
          ['Total Policies',  s.totalPolicies,  'var(--accent)'],
          ['Active Policies', s.activePolicies, 'var(--green)'],
          ['Total Claims',    s.totalClaims,    '#7C6BFF'],
          ['Open Claims',     s.openClaims,     'var(--red)'],
          ['Total Users',     s.totalUsers,     'var(--teal)'],
          ['Platform Fee',    summary.totalPlatformFee != null ? `₦${Number(summary.totalPlatformFee).toLocaleString()}` : '—', 'var(--muted-light)'],
        ].map(([l, v, c]: any) => (
        ...
        <div className="flex flex-wrap justify-between items-center gap-3 mb-4">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            <button onClick={exportCsv}
              ...
              style={{ background: 'rgba(0,194,168,.12)', color: 'var(--teal)', border: '1px solid rgba(0,194,168,.3)' }}>
              ↓ Export CSV
            </button>
            {selected.size > 0 && (
              ...
              <tbody>
                {visible.map((c: any) => (
                  <tr key={c.id} className="transition-all hover:bg-white/5"
                    ...
                    <td className="py-3 pr-4 font-semibold" style={{ color: 'var(--accent)' }}>₦{Number(c.grossPremium || 0).toLocaleString()}</td>
                    <td className="py-3 pr-4 text-muted text-xs">{c.commissionRate ? (Number(c.commissionRate) * 100).toFixed(0) + '%' : '—'}</td>
                    <td className="py-3 pr-4 font-bold" style={{ color: 'var(--green)' }}>₦{Number(c.commissionAmount || 0).toLocaleString()}</td>
                    <td className="py-3 pr-4 text-xs text-muted">₦{Number(c.platformFee || 0).toLocaleString()}</td>
                    <td className="py-3 pr-4 font-semibold" style={{ color: 'var(--teal)' }}>₦{Number(c.netCommission || 0).toLocaleString()}</td>
                        ...
                        {c.status === 'pending' && (
                          <>
                            ...
                            <button onClick={() => openNoteModal(c.id, 'paid')} disabled={acting}
                              className="px-2 py-1 rounded-lg text-xs font-semibold disabled:opacity-40 hover:brightness-110 transition-all"
                              style={{ background: 'rgba(46,201,126,.12)', color: 'var(--green)', border: '1px solid rgba(46,201,126,.3)' }}>
                              Mark Paid
                            </button>
                          </>
                        )}
