...
export class GroupPolicy {
  ...
  @Column({ name: 'owner_id' }) ownerId: string; // SME owner
  ...
  @Column({ name: 'policy_name' }) policyName: string; // e.g. "Staff Health Plan Q1 2026"
  @Column({ name: 'policy_type' }) policyType: string; // fleet | group_life | group_health | group_fire
  ...
  @Column({ default: 'active' }) status: string; // draft | active | expired | cancelled

  @Column({ name: 'start_date', type: 'date', nullable: true }) startDate: Date;
  @Column({ name: 'end_date', type: 'date', nullable: true }) endDate: Date;

  @Column({ type: 'jsonb', default: [] }) members: GroupMember[]; // stored as JSON array
